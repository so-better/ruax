class Ruax {
    ...
    _getValidationConfig(config) {
        ...
        if (
            ...
            (config.type.toLocaleUpperCase() != 'POST' &&
                config.type.toLocaleUpperCase() != 'GET')
        ) {
    ...
    _setDefaults() {
        ...
        //type请求方式post/get
        if (
            typeof this.defaults.type != 'string' ||
            (this.defaults.type.toLocaleUpperCase() != 'POST' &&
                this.defaults.type.toLocaleUpperCase() != 'GET')
        ) {
            this.defaults.type = 'GET'
        }
        //timeout请求超时时间
        if (
            typeof this.defaults.timeout != 'number' ||
            isNaN(this.defaults.timeout)
        ) {
            this.defaults.timeout = 8000
        }
        //dataType返回参数类型
        if (
            typeof this.defaults.dataType != 'string' ||
        ...
        //ContentType配置
        if (
            typeof this.defaults.contentType != 'string' &&
            this.defaults.contentType !== false
        ) {
                ...
                if (xhr.readyState == 4) {
                    ...
                    if (xhr.status == 200) {
                        ...
                        if (config.dataType.toLocaleLowerCase() == 'json') {
                            ...
                            config.dataType.toLocaleLowerCase() == 'xml'
                        ) {
                            res = xhr.responseXML
                        } else if (
                            config.dataType.toLocaleLowerCase() == 'html' ||
                            config.dataType == 'string'
                        ) {
                            res = xhr.responseText
                        } else if (
                            config.dataType.toLocaleLowerCase() == 'blob'
                        ) {
                            res = xhr.response
                        } else {
                            res = xhr.responseText
